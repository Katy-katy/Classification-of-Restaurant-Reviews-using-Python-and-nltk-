...
def add_lexical_features(fdist, bidist, feature_vector, text):
	...
	#unigram features
	for word, freq in fdist.items(): 
		fname = "UNI_" + word + "_" + str(bin(freq))
		if selected_features == None or fname in selected_features:
	...
	#bigram features
	for word, freq in bidist.items(): # fdist = nltk.FreqDist(review_words)
		fname = "BIGRAM_" + word[0] + "_" + word[1] + "_" + str(bin(freq))
		if selected_features == None or fname in selected_features:
			feature_vector[fname] = 1           
			#num_of_w = text_nl.count(word)
	...
	for word, freq in fdist_pos.items(): 
		fname = "UNIPOS_" + word + "_" + str(bin(freq))
		if selected_features == None or fname in selected_features:
			feature_vector[fname] = 1            
			#num_of_w = text_nl.count(word)
	...
	for word, freq in bidist_pos.items(): 
		fname = "BIPOS_" + word[0] + "_" + word[1] + "_" + str(bin(freq))
		if selected_features == None or fname in selected_features:
...
#python3 restaurant-competition-P2.py restaurant-training.data restaurant-development.data restaurant-testing.data
if __name__ == '__main__':
    ...
    accuracy = nltk.classify.accuracy(classifier, featuresets3)
    print("{0:6s} {1:8.5f}".format("Test", accuracy))
    evaluate(classifier, featuresets3, texts, "restaurant-testing.data", "output-best.txt") 
